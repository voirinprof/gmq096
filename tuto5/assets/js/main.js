...
// on va gérer l'interaction avec la souris, 
// ainsi dès que je clique sur le logo, on va incrémente un compteur
...
var score = 0; // initialisation du score (variable globale)
...
function create () // méthode de création des objets
{
    ...
    let mytext = this.add.text(10, 10, 'Score: '+score, { fontSize: '22px', fill: '#fff' })


    // comme on souhaite gérer l'interaction avec la souris
    // on va activer l'interaction avec le logo
    logo.setInteractive(); // activer l'interaction avec le logo
    
    // on va écouter l'événement pointerdown
    logo.on('pointerdown', function () {
        // dès que l'événement pointerdown est déclenché, on ajoute 10 points au score
        score += 10;
